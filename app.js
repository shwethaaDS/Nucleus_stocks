...
const assemble_computer = require("./routes/assemble_computer");
...
app.get("/biometric", function(req, res) {
  var rowCount = 5; // or whatever value you want rowCount to have
  res.render('biometric', { rowCount: rowCount });
});
...
// Handling GET request for /addstock
    ...
    case "lab_assistant":
      db.query("SELECT DISTINCT devicetype FROM device", (err, result) => {
        if(err){
          ...
          res.render("addstock", {
            title: "Add Stock",
            devices: deviceTypes, // You can pass an empty array or other necessary data
            role: res.locals.role,
            isPR: res.locals.isPR,
          });
        }
...
// Handling POST request for /addstock
...
// Handling POST request for /add_device
...
app.get("/add_computer", getCookie.getCookie, (req, res) => {
  if (res.locals.role === "lab_assistant")
...
app.post("/add_computer", getCookie.getCookie, (req, res) => {
  if (res.locals.role === "lab_assistant") {
    addstock.add_computer(req, res);
  } else {
    res.render("denial");
  }
});
app.get("/auth", getCookie.getCookie, (req, res) => {
  if (res.locals.role === "admin") addstock.auth(req, res);
  else res.render("denial");
});

app.post("/verify-stock", getCookie.getCookie, (req, res) => {
  if (res.locals.role === "admin") addstock.authorizeStock(req, res);
  else res.render("denial");
});
...
app.get("/assemble_computer", getCookie.getCookie, (req, res) => {
  switch (res.locals.role) {
    case "admin":
    case "lab_assistant":
    case "teacher":
...
app.post("/assemble_computer", getCookie.getCookie, (req, res) => {
  switch (res.locals.role) {
    case "admin":
    case "lab_assistant":
    case "teacher":
      assemble_computer.assemble_computer(req, res);
      break;
    default:
      res.render("denial");
  }
...
app.get("/authorize_dump", getCookie.getCookie, (req, res) => {
  if (res.locals.role === "admin") dump.authorize_dump(req, res);
  else res.render("denial");
});
...
app.post("/verify-dump", getCookie.getCookie, (req, res) => {
  if (res.locals.role === "admin") dump.authorizeDumpList(req, res);
  else res.render("denial");
});
